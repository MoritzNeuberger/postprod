...
class data_manager:
    ...
    def process_data(self):
        ...
        for batch, report in self.ttree.iterate(step_size=self.inst["para"]["step_size"],report=True):
            pbar.update(report.stop - report.start)
            processing_variables = {key: batch[value.rsplit("/")[-1]] for key, value in self.inst["input"]["var"].items()}
    ...
    def write_output(self):
        with h5py.File(self.outfile, "w") as f:
            ...
            form, length, container = ak.to_buffers(ak.to_packed(self.output_dict), container=group)
