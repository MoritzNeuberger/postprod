...
def is_point_inside_polycone(x, y, z, r_values, z_values):
    ...
    for i in range(len(z_values) - 1):
        ...
        if z_low <= z <= z_high:
            ...
            t = (z - z_low) / (z_high - z_low)  # Normalized z position between z_low and z_high
...
def generate_mask_deadlayer(x,y,z,vol,para):
    ...
    with open(para["file"],"r") as f:
        dl_input = json.load(f)

        
    def convert_to_numba_dict(py_dict):
        # Create the outer numba.typed.Dict
        nb_dict = Dict.empty(key_type=types.int64, value_type=types.DictType(types.unicode_type, types.float64[:]))

        for vol_key, vol_value in py_dict.items():
            # Create the inner dictionary for each volume (flatten surface_mesh fields)
            vol_dict = Dict.empty(key_type=types.unicode_type, value_type=types.float64[:])
            ...
            r_orig_array = np.array(vol_value['surface_mesh']['orig']['r'], dtype=np.float64)
            z_orig_array = np.array(vol_value['surface_mesh']['orig']['z'], dtype=np.float64)
            r_dl_array = np.array(vol_value['surface_mesh']['dl']['r'], dtype=np.float64)
            z_dl_array = np.array(vol_value['surface_mesh']['dl']['z'], dtype=np.float64)
        ...
        for i in range(len(x)): #event
            ...
            for j in range(len(x[i])): #window
                ...
                for k in range(len(x[i][j])): #detector
                    tmp3 = list()
                    for l in range(len(x[i][j][k])): #hit
                        if vol[i][j][k][l] > 1000000:
                            tmp3.append(is_in_active_volume(x[i][j][k][l],y[i][j][k][l],z[i][j][k][l],vol[i][j][k][l],dl_input))
                    tmp2.append(tmp3)
                tmp.append(tmp2)
            output.append(tmp)
        return output
...
def m_active_volume(para, input, output, pv):
    ...
    if para["type"] == "cylinder":
        mask = generate_mask_cylinder(pv[in_n["posx"]],pv[in_n["posy"]],pv[in_n["posz"]],para)
    ...
    if para["type"] == "deadlayer":
        mask = generate_mask_deadlayer(pv[in_n["posx"]],pv[in_n["posy"]],pv[in_n["posz"]],pv[in_n["vol"]],para)
        pv[out_n["vol_red"]] = ak.firsts(pv[in_n["vol"]],axis=-1)

        for key in in_n.keys():
            tmp = pv[in_n[key]][mask]
            mask_tmp = ak.Array([[[True for arr3 in arr2 if len(arr3)] for arr2 in arr1] for arr1 in tmp])
